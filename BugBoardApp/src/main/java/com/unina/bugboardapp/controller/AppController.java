...
/**
 * Facciata (Facade) e punto di accesso centrale ai controller
 * dell'applicazione.
 * <p>
 * Questa classe incapsula lo stato applicativo ({@link AppState}) e delega le
 * operazioni
 ...
 *
 * <h2>Pattern</h2>
 * Implementa il pattern Singleton tramite {@link #getInstance()} per garantire
 * una sola istanza
 * condivisa nell'applicazione.
 *
 * <h2>Note sul threading</h2>
 * Alcune operazioni delegate (es. refresh, creazione issue/utente/commenti)
 * possono essere eseguite
 * asincronamente dai controller sottostanti (tipicamente con aggiornamenti UI
 * tramite JavaFX).
 */
public class AppController {
     ...
     * <p>
     * In caso di autenticazione riuscita, lo stato applicativo viene aggiornato con
     * l'utente loggato
     * e può essere innescato un refresh dei dati (delegato al controller di
     * autenticazione).
     * </p>
     *
     * @param email    email/username dell'utente
     * @param password password dell'utente
     * @return {@code true} se l'autenticazione va a buon fine, {@code false}
     *         altrimenti
     */
     ...
     * <p>
     * Delegato a {@link AuthenticationController}, che azzera l'utente nello
     * {@link AppState}.
     * </p>
     */
    public void logout() {
     ...
     *
     * @return {@code true} se l'utente loggato esiste ed ha tipo
     *         {@link UserType#ADMIN},
     ...
     * <p>
     * Le validazioni (es. utente loggato, campi non null/blank) sono gestite dal
     * controller delegato.
     * </p>
     *
     ...
     * @param imagePath   percorso immagine associata (opzionale, può essere
     *                    {@code null} o vuoto)
     * @param state       stato iniziale della issue
     * @throws IllegalStateException    se l'utente non è loggato (in base alle
     *                                  regole del controller delegato)
     * @throws IllegalArgumentException se i parametri non rispettano i vincoli di
     *                                  validazione
     */
    ...
    /**
     * Restituisce la lista osservabile di tutte le issue presenti nello stato
     * applicativo.
     * <p>
     * La lista è quella mantenuta da {@link AppState}; eventuali aggiornamenti (es.
     * refresh)
     ...
     *
     * @param type tipo di issue su cui filtrare; se {@code null} viene restituita
     *             una copia
     ...
     *
     * @param priority priorità su cui filtrare; se {@code null} viene restituita
     *                 una copia
     *                 contenente tutte le issue
     ...
     *
     * @param state stato su cui filtrare; se {@code null} viene restituita una
     *              copia
     *              contenente tutte le issue
     ...
     * @throws IllegalStateException    se l'utente corrente non è amministratore
     *                                  (in base alle regole del controller
     *                                  delegato)
     * @throws IllegalArgumentException se i parametri non sono validi o l'utente
     *                                  risulta già presente nello stato
     */
     ...
     * <p>
     * La logica (incluse eventuali scelte "fail-safe" in caso di errori) è delegata
     * a
    ...
    /**
     * Restituisce la lista osservabile di tutti gli utenti presenti nello stato
     * applicativo.
     *
     ...
     * <p>
     * L'operazione può essere asincrona; in caso di successo, il commento creato
     * può essere
     * inserito nella issue e la callback {@code onSuccess} (se non {@code null})
     * viene invocata.
     * </p>
     *
     ...
     * @param onSuccess callback opzionale invocata con il {@link Comment} creato
     *                  con successo
     * @throws IllegalStateException    se l'utente non è loggato (in base alle
     *                                  regole del controller delegato)
     * @throws IllegalArgumentException se {@code issue} è {@code null} o
     *                                  {@code content} non è valido
     */
    ...
    /**
     * Carica i commenti associati ad una issue delegando a
     * {@link CommentController}.
     * <p>
     * L'operazione è tipicamente asincrona; al completamento, i commenti ottenuti
     * vengono impostati
     * sulla issue.
     * </p>
     *
